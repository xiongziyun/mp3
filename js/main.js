$(function () {
  ...
  // 收藏列表
  let collectListArr = localStorage.collectListArr
    ? JSON.parse(localStorage.collectListArr)
    : [];
  // 全部列表
  let allListArr = [];
  ...
  $(".handle-list a").click(function () {
    const text = $(this).text();
    musicListArr = text === "全部" ? allListArr : collectListArr;
    init()
  });
  function init() {
    $(".music-list").html(`
    <li>
      <div class="music-item-number"><a></a></div>
      <div class="music-item-name"><a>歌曲名</a></div>
      <div class="music-item-singer"><a>歌手名</a></div>
      <div class="music-item-time"><span>时间</span></div>
    </li>
    `);
    $.each(musicListArr, function (index, item) {
      createMusicListDOM(item, index);
  ...
  function initEvent() {
    ...
    $(".music-list").on("click", ".music-item-delete", function () {
      ...
      $(".music-list-item").each(function (index, element) {
        element.index = index;
        $(element)
          .find(".music-item-number")
          .html(index + 1);
      });
    });

    // 加入收藏
    $(".music-list").on("click", ".music-item-add", function () {
      // collectListArr
      ...
      collectListArr.push({
        ...
        album: "",
        time: "",
        link_url: `./source/${name}.mp3`,
        cover: `./source/${name}.jpg`,
        link_lrc: `./source/${name}.txt`,
      });
      localStorage.collectListArr = JSON.stringify(collectListArr);
    });
      ...
      switch (playMode) {
        ...
        case "randomCirculation":
          ...
          let randomNum = Math.floor(
            Math.random() * $(".music-list-item").length
          );
          index = randomNum;
          break;
      }
  ...
  function createMusicListDOM(obj, index) {
    let $li = $(`
      <li class="music-list-item">
        ...
        <div class="music-item-singer"><a href="javascript:;">${
          obj.singer
        }</a></div>
        <div class="music-item-time"><span>${
          obj.time
        }</span><a href="javascript:;" class="music-item-delete a-i-fade-show"></a></div>
      </li>
    `);
  ...
  window.search = function () {
    ...
    musicListArr = allListArr.filter(v => v.name.includes(name))
